...
public class adminappointmentchange extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (value.equals("Change"))
        {
            ...
            request.getRequestDispatcher("searchdoctors1").forward(request, response);
        }
        if (value.equals("Cancel"))
        {
            HttpSession session = request.getSession();
            session.setAttribute("fail", "out");
            if(a.deleteappointment(paID)){
            request.getRequestDispatcher("adminappointments").forward(request, response);
            }
        }
    }
