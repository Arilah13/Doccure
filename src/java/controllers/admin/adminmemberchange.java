 ...
 * @author rilah
 */
public class adminmemberchange extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if(request.getParameter("change").equals("Edit"))
        {
            ...
            b.setDid(did);
            int c = a.updateuser(b);
            if(c >0)
            {
                HttpSession session = request.getSession();
            session.setAttribute("update", "success");
                request.getRequestDispatcher("adminmembers").forward(request, response);
            }
            else
            {
                HttpSession session = request.getSession();
