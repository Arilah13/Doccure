...
public class staffprofileupdate extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try{     
            if(password == null)
              {
              PreparedStatement ps = con.createConnection().prepareStatement("update staff SET pic = ?, firstname = ?, lastname = ?, phonenumber = ?, gender = ?, dateofbirth = ?, piccode = ?, email = ? where id = ?");
              if (inputStream != null){
              ...
              {
                  PreparedStatement ps = con.createConnection().prepareStatement("update staff SET pic = ?, firstname = ?, lastname = ?, phonenumber = ?, gender = ?, dateofbirth = ?, piccode = ?, email = ?, password = ?, salt =? where id = ?");
                    if (inputStream != null){
                    ps.setBinaryStream(1, inputStream, (int)filePart.getSize());
                    }
                     ps.setString(2, fname);
              ...
              PreparedStatement ps1 = con.createConnection().prepareStatement("select * from staff where id = ?");
              ...
              if (x > 0){
                ...
                List<GS> appointments = u.getstaffdetails(Integer.parseInt(cookie));
